...
**Goal:** Build the complete `AdityaJogdand/AdityaJogdand` GitHub profile repo — animated SVG banner, terminal-style ANSI-colored README content, and GitHub stats — matching the approved design spec.
...
**Tech Stack:** Hand-authored SVG + CSS `@keyframes`/SMIL, GitHub-flavored Markdown with `ansi` code fences, Python 3 (stdlib only) for the README generator, third-party badge services (`github-readme-stats`, `streak-stats.demolab.com`) for stats cards.
...
### Task 7: [REMOVED] Contribution snake workflow

This task originally added `.github/workflows/snake.yml` (a scheduled GitHub Action generating a live "snake eats the contribution graph" SVG). The user decided against it before this task was implemented — no commits exist for it, nothing to revert. `.github/` does not appear in the final repo. Task 8's GitHub Stats table and its test assertions have been updated below to no longer reference the snake image.
...
- [ ] **Step 5: Replace the resume placeholder**
